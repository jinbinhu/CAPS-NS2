...
puts "Creating Links..."
for {set i 1000} {$i < [expr $shortfno+1000]} {incr i} {
	$ns simplex-link $n($i) $n(0) $lineRate [expr $RTT/8] DropTail
	$ns simplex-link  $n(0) $n($i) $lineRate [expr $RTT/8] DropTail
	$ns queue-limit $n($i) $n(0) 2000
}

for {set i 2000} {$i < [expr $longfno+2000]} {incr i} {
	$ns duplex-link $n($i) $n(0) $lineRate [expr $RTT/8] DropTail
	$ns queue-limit $n($i) $n(0) 2000
}

for {set i 1} {$i < [expr $pathNum+1] } {incr i} {
	$ns duplex-link $n(0) $n($i) $lineRate [expr $RTT/8] DropTail  
	$ns queue-limit $n(0) $n($i) 256
}

for {set i 1} {$i < [expr $pathNum+1] } {incr i} {
	set sinkno [expr $pathNum+1]
	$ns simplex-link $n($i) $n($sinkno) $lineRate [expr $RTT/8] DropTail
	$ns simplex-link $n($sinkno) $n($i) $lineRate [expr $RTT/8] DropTail
	$ns queue-limit $n($i) $n($sinkno) 256 
}

	set sinkno [expr $pathNum+1]
	$ns duplex-link $n($sinkno) $n(3000) $lineRate [expr $RTT/8] DropTail
